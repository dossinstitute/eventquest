...
describe("UserManager", function () {
    ...
    beforeEach(async function () {
        ...
        userManager = await UserManagerFactory.deploy(await questManager.getAddress());
    ...
    describe("Quest Registration", function () {
        ...
        it("Should allow a registered user to register for a quest", async function () {
            await userManager.connect(user1).registerForQuest(1);
        ...
        it("Should not allow an unregistered user to register for a quest", async function () {
            await expect(userManager.connect(user2).registerForQuest(1))
                .to.be.revertedWith("User not registered");
        });

        it("Should not allow a user to register for a non-existent quest", async function () {
            await expect(userManager.connect(user1).registerForQuest(2))
